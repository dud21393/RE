...
<form action="../Ctl/MainCTL.php?action=1022" method="post" style="text-align: right; padding-right: 26%">
    <input type="submit" value="상품입력">
</form>
<table border="1" align="center">
    <tr><td>번호</td><td>카테고리</td><td>상품코드</td><td>상품이름</td><td>재고량</td><td>가격</td><td>큰이미지</td><td>작은이미지</td><td>수정</td><td>삭제</td></tr>
    <?php
    ...
    if($product_Listinfo) {
        for ($iCount = 0; $iCount < count($product_Listinfo); $iCount++) {
            ...
            echo "<td><a href='../Ctl/MainCTL.php?action=1027&pnum={$product_Listinfo[$iCount][0]}&page={$product_pageinfo['current_page']}'>수정</a></td>";
            echo "<td><a href='../Ctl/MainCTL.php?action=1026&page={$product_pageinfo['current_page']}&product_num={$product_Listinfo[$iCount][0]}&pfimage={$product_Listinfo[$iCount][6]}&psimage={$product_Listinfo[$iCount][7]}'>삭제</a></td>";
            echo "</tr>";
        }
    }else{
